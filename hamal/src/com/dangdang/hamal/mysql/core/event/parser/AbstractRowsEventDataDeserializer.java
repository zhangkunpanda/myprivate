...
public abstract class AbstractRowsEventDataDeserializer<T extends EventData> implements EventDataDeserializer<T> {
     ...
     * @throws IOException
     */
    protected Serializable[] deserializeOneRow(long tableId, BitSet includedColumns, ByteArrayInputStream inputStream)
            throws IOException {
        ...
        Serializable[] cells = new Serializable[numberOfBitsSet(includedColumns)];
        BitSet nullColumns = inputStream.readBitSet(cells.length, true);
        for (int i = 0, numberOfSkippedColumns = 0; i < types.length; i++) {
            ...
            if (!nullColumns.get(index)) {
                cells[index] = deserializeCell(ColumnType.byCode(typeCode), meta, length, inputStream);
            }
        }
        return cells;
    }
